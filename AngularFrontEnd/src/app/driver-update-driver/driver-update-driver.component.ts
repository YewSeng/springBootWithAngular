...
@Component({
  selector: 'app-driver-update-driver',
  templateUrl: './driver-update-driver.component.html',
  styleUrls: ['./driver-update-driver.component.css']
})
export class DriverUpdateDriverComponent implements OnInit {
  ...
  updateDriver() {
    ...
    const password = this.updateDriverForm.get('password').value;
        ...
        next: (result: { usernameTaken: boolean, emailTaken: boolean, mobileNumberTaken: boolean }) => {
            ...
            if (!this.usernameTaken && !this.emailTaken && !this.mobileNumberTaken) {
                ...
                this.driverService.updateDriver(userId, driverToUpdate).subscribe({
                    next: () => {
                        this.router.navigate(['/driver/viewProfile/' + this.driverId], { state: { success: true } });
                    },
                    error: (error) => {
